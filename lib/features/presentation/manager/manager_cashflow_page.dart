...
/// Page showing today's cash flow summary for all sales (manager view)
class ManagerCashflowPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Today's Cash Flow"),
        centerTitle: true,
        elevation: 0,
                        ...
                        children: [
                          _InfoCard(
                            ...
                            stats: [
                              ...
                              'Paid: ${lbp.format(revenue)}',
                              'Deposits: ${lbp.format(depositTotal)}',
                            ],
                          ),
                          _InfoCard(
                            ...
                            title: 'Cash Flow',
                            stats: [
                              'Collected: ${lbp.format(cashIn)}',
                              'New: ${lbp.format(revenue)}',
                              'Past: ${lbp.format(cashIn - revenue)}',
                            ],
                          ),
                          _InfoCard(
                            icon: Icons.receipt_long_outlined,
                            title: 'Receivables',
                            stats: ['Owed: ${lbp.format(owedTotal)}'],
                          ),
                        ],
                      ),
                      const SizedBox(height: 24),
                      Text('Items Sold Today', style: Theme.of(context).textTheme.titleLarge),
                      ...
                      ...itemsSold.entries.map((e) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text('Total $total • Paid $paid • Depo $depo • Unpaid $unp'),
                          ),
                        );
                      }),
                      const SizedBox(height: 24),
                      Text('Unpaid Customers', style: Theme.of(context).textTheme.titleLarge),
                      const Divider(thickness: 1.5),
                      if (oweMap.isEmpty)
                        const Padding(
                          padding: EdgeInsets.symmetric(vertical: 16),
                          child: Center(child: Text('None', style: TextStyle(fontSize: 16))),
                        ),
